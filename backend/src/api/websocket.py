"""
WebSocket endpoint for real-time Q&A extraction processing.

Endpoint: WS /api/process

Protocol (contracts/api.md §2):
  ...
  2. Server validates the payload, resolves the API key and prompt config,
     splits each file into chunks, and enqueues them in FIFO order.
  3. Server processes each chunk sequentially, streaming events back:
       - LOG          → real-time informational / error log entry
       - CHUNK_SUCCESS → chunk processed, extracted pairs included
...
Design notes:
  - A single asyncio queue (asyncio.Queue) is used as the FIFO work list.
  - Key storage and prompt storage are accessed via their respective service modules
    (resolved lazily to avoid circular imports during incremental development).
  - The WebSocket router is registered by main.py under the "Processing WebSocket" tag.
"""
...
async def _resolve_api_key(key_id: str) -> str:
    ...
    sentinel value 'env', enabling development without persistent storage.
...
async def _resolve_prompt_config(prompt_id: str) -> PromptConfig | None:
    """
    Retrieve the PromptConfig for *prompt_id* from the prompt storage service.
    Returns None if the module is not yet implemented (uses default prompt).
    """
    ...
    try:
        ...
        logger.warning(
            "Prompt ID '%s' not found in storage. Using default prompt.", prompt_id
        )
        return None


# ──────────────────────────────────────────────────────────────────────────────
# Core FIFO queue processor
# ──────────────────────────────────────────────────────────────────────────────
...
async def _process_queue(
    ...
    for fp in files:
        ...
        raw_text = parser.parse(fp.conteudoBruto.encode("utf-8"))
    ...
    while not queue.empty():
        ...
        await _send_log(
            ws,
            f"Processando chunk {chunk_idx + 1}/{total} de '{arquivo.nomeArquivo}'…",
        )
        ...
        try:
            ...
            # Rate-limit exhaustion: halt queue and report partial results
...
@router.websocket("/api/process")
async def process_websocket(ws: WebSocket) -> None:
    """
    WebSocket endpoint that accepts a START message and streams processing
    ...
    try:
        ...
        await _send_log(
            ws,
            f"Sessão iniciada. {len(start_msg.files)} arquivo(s) recebido(s).",
        )
        ...
        # ── 3. Resolve prompt config ──────────────────────────────────────────
        prompt_config = await _resolve_prompt_config(start_msg.prompt_id)
        ...
        # ── 4. Run the FIFO queue processor ──────────────────────────────────
